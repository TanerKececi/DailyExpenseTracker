...
- **Never paint a foreground element in `@color/background`** (renamed from `background_light_gray` in #13). It is the app's own screen background, so anything drawn in it is invisible. This has now shipped as a bug three separate times — Bills' selected tab label, the Calendar divider, and Budget Planner's progress tracks. Use `@color/divider` for hairlines and empty track fills. None of these were caught by a build or a test; only by looking at a screenshot.
